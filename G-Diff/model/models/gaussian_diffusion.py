...
class GaussianDiffusion(nn.Module):
    ...
    def training_losses(self, model, x_start, sample_stype='importance', reweight=False):
        batch_size, device = x_start.size(0), x_start.device
        ts, pt = self.sample_timesteps(batch_size, device, sample_stype)
        ...
        # bce = torch.binary_cross_entropy_with_logits(input=model_output, target=target.bool().float(), weight=target)
        # mse = mse + bce
