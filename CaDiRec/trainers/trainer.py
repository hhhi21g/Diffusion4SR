...
from utils import get_full_sort_score, hrs_and_ndcgs_k, EarlyStopping
...
class Trainer:
    ...
    def train(
        ...
        if eval_interval is None:
            eval_interval = getattr(self.args, "eval_interval", 10)
        ...
        for epoch in trange(self.start_epoch, self.start_epoch + int(self.args.epochs), desc="Epoch"):
            ...
            if epoch % eval_interval == 0:
                valid_metrics = self.eval(epoch, test=False)  # valid

                valid_score = valid_metrics.get(objective_metric, 0.0)
                if valid_score > best_valid_score:
                    best_valid_score = valid_score
                    best_epoch = epoch
                    best_valid_metrics = valid_metrics
                    no_improve_rounds = 0
                    if run_test_during_tuning:
                        best_test_metrics = self.eval(epoch, test=True)  # test
                else:
                    no_improve_rounds += 1

                if trial is not None and enable_pruning:
                    trial.report(valid_score, step=epoch)
                    if trial.should_prune():
                        import optuna
                        raise optuna.TrialPruned()

                if early_stop_rounds is not None and early_stop_rounds > 0:
                    if no_improve_rounds >= early_stop_rounds:
                        print(
                            f"Early stop at epoch {epoch}: "
                            f"no improvement on {objective_metric} for {early_stop_rounds} eval rounds."
                        )
                        break
    ...
    def eval(self, epoch, test=False):
        ...
        return get_full_sort_score(epoch, metrics_dict)
