...
from datasets import (CausalLMDataset, CausalLMPredictionDataset,
                      ...
                      PaddingCollateFn)
from metrics import compute_hr_ndcg_metrics, compute_sampled_metrics
...
EVAL_METRIC_KS = [1, 5, 10, 20]


def metrics_to_percent(metrics, decimals=5):
    return {key: round(value * 100, decimals) for key, value in metrics.items()}


def metrics_to_display_strings(metrics, decimals=5):
    return {key: f'{value:.{decimals}f}' for key, value in metrics.items()}
...
def predict(trainer, seqrec_module, data, config):
    ...
    seqrec_module.predict_top_k = max(EVAL_METRIC_KS)
...
def evaluate(recs, test, train, seqrec_module, dataset, task, config, prefix='test'):
    ...
    base_metrics = compute_hr_ndcg_metrics(test, recs, ks=EVAL_METRIC_KS)
    prefixed_metrics = {f'{prefix}_{key}': value for key, value in base_metrics.items()}
    all_metrics = metrics_to_percent(prefixed_metrics)
    print(metrics_to_display_strings(all_metrics))
    ...
    if config.sampled_metrics:
        ...
        uniform_metrics = metrics_to_percent(uniform_metrics)
        print(metrics_to_display_strings(uniform_metrics))
        ...
        popularity_metrics = metrics_to_percent(popularity_metrics)
        print(metrics_to_display_strings(popularity_metrics))
